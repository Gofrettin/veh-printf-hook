...
LONG WINAPI veh(EXCEPTION_POINTERS *exception_info) {
    ...
    if(exception_info->ExceptionRecord->ExceptionCode == STATUS_SINGLE_STEP) {
        DWORD prot_buff;
        VirtualProtect((LPVOID)printf, 1, PAGE_EXECUTE_READ | PAGE_GUARD, &prot_buff);
        return EXCEPTION_CONTINUE_EXECUTION;
    }
...
int main() {
    ...
    if(!VirtualQuery((const uintptr_t*)printf, &printf_info, sizeof(printf_info))) { printf("Failed to query basic information with VirtualQuery.\nGLE(get last error) code: %i", GetLastError()); return 0; }
    if(!VirtualQuery((const uintptr_t*)print_woo, &printwoo_info, sizeof(printwoo_info))) { printf("%s", "Failed to query basic information with VirtualQuery.\nGLE(get last error) code: %i", GetLastError()); return 0;}
    ...
    if(!VirtualProtect((LPVOID)printf, 1, PAGE_EXECUTE_READ | PAGE_GUARD, &prot_buff)) { printf("Failed to enable page guard with VirtualProtect.\nGLE(get last error) code: %i", GetLastError()); return 0;}
